import React from 'react';
import styles from './BookingStepper.module.css';

const steps = [
    { id: 1, label: 'Search' },
    { id: 2, label: 'Passenger Details' },
    { id: 3, label: 'Baggage' },
    { id: 4, label: 'Meal' },
    { id: 5, label: 'Seating' },
    { id: 6, label: 'Overview & Payment' },
];

export default function BookingStepper({ currentStep = 3 }) {
    // Calculate progress line percentage based on currentStep
    // If currentStep is 3, line goes from 1 to 3.
    const progressPercentage = ((currentStep - 1) / (steps.length - 1)) * 100;

    return (
        <div className={styles.stepperWrapper}>
            <div className={styles.stepperContainer}>
                {/* Steps */}
                {steps.map((step, index) => {
                    const isActive = step.id === currentStep;
                    const isCompleted = step.id < currentStep;

                    return (
                        <div
                            key={step.id}
                            className={`${styles.stepItem} ${isActive ? styles.stepActive : ''} ${isCompleted ? styles.stepCompleted : ''}`}
                        >
                            <div className={styles.circleWrapper}>
                                {(isActive || isCompleted) && <div className={styles.outerRing}></div>}
                                <div className={styles.stepCircle}>
                                    {isCompleted ? (
                                        <svg className={styles.checkmark} viewBox="0 0 14 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M1 5.5L5 9.5L13 1.5" />
                                        </svg>
                                    ) : (
                                        step.id
                                    )}
                                </div>
                            </div>
                            <span className={styles.stepLabel}>{step.label}</span>
                        </div>
                    );
                })}
            </div>
        </div>
    );
}
